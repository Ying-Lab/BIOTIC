...
class tre(nn.Module):
    ...
    def setup_networks(self):
        ...
        self.encoder_ptf = MLP(#xs->ptf zk
    ...
    def model(self, xs,ys = None, acc_p = None,barcode = None):
        ...
        with pyro.plate('data'):
            ...
            tf_loc = torch.zeros(batch_size, self.mask_size, **options)+(self.mask*0.5).ravel()
    ...
    def guide(self, xs, ys = None,acc_p = None,barcode = None):
        ...
        with pyro.plate('data'):
            ...
            #tf_loc, tf_scale = self.encoder_mtf([ptf, mask.ravel()])
            #tf = pyro.sample('tf', dist.Normal(tf_loc, tf_scale).to_event(1))
    ...
    def latent_embedding(self, xs):
        ...
        zs_norm = (zs - mu) / sigma 
        #zs=self.zscore_normalize_features(zs)
        return zs_norm    
    ...
    def denoised_expression(self, xs, acc_p = None, l=10):
        """
        ...
        zs,_ = self.encoder_z(xs)
        tf,_ = self.encoder_ptf(xs)
        concentrate = self.decoder_thetas([zs, tf,acc_p])
